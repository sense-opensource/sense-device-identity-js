...
export interface FontDetection {
	fonts: string[]; // Array of detected fonts
	fontHash: string; // Hash of the detected fonts
	fontsCount: number; // Count of detected fonts,
}

// Font detection utility
export const getFonts = (): FontDetection => {
	...
	for (let i = 0; i < m.length - 1; i++) {
		const n: string[] = ['monospace', 'sans-serif', 'serif'];
		...
		const text: string = 'abcdefghijklmnopqrstuvwxyz0123456789';
		...
		for (let j = 0; j < n.length; j++) {
			...
			const baselineSize: number = context.measureText(text).width;

			// Measure with the custom font and fallback.
			context.font = `72px '${m[i]}', ${n[j]}`;
			const newSize: number = context.measureText(text).width;
